...
void MultiBillboard::drawCPUBillboards(QGLPainter *painter) {
    ...
    int numberOfAtoms = 0;
    for(DataBundle* bundle : *(m_dataSource->dataBundles())) {
        numberOfAtoms += bundle->positions().size();
    }
    ...
    for(DataBundle* bundle : *(m_dataSource->dataBundles())) {
        const QArray<QVector3D> &positions = bundle->positions();
        const QColor4ub &color = bundle->color();
        const QVector2D &size = bundle->size();

        for(int i = 0; i <positions.count(); i++) {
            const QVector3D &center = positions.at(i);
            a = center + size.y()*aOffset_x + size.x()*aOffset_y;
            b = center + size.y()*bOffset_x + size.x()*bOffset_y;
            c = center + size.y()*cOffset_x + size.x()*cOffset_y;
            d = center + size.y()*dOffset_x + size.x()*dOffset_y;
            vertices.append(a, b, c, d);
            colorArray.append(color, color, color, color);
